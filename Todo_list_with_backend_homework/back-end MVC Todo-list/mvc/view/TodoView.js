...
TodoView.prototype.render = function(data) {
    ...
    $list.on('click', '.delete-item', function(e) {
        typeof self.onDelete === 'function' && self.onDelete(data, getElement(data, e).id);
    });

    $list.on('change', '.chck', function(e) {
        typeof self.onComplete === 'function' && self.onComplete(data, getElement(data, e).id, e.currentTarget.checked);
    })
